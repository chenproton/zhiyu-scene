...
function useAnnotations(config) {
    ...
    const refreshAnnotations = (0, react_1.useCallback)(() => {
        if (!page)
            return;
        setError(null);
        fetch(`${apiBasePath}/annotations?page=${encodeURIComponent(page)}&context=${encodeURIComponent(context || 'default')}`)
    ...
    const refreshComments = (0, react_1.useCallback)((annotationId) => {
        setError(null);
    ...
    const createAnnotation = (0, react_1.useCallback)(async (x, y, content, imageUrl) => {
        try {
    ...
    const updateAnnotation = (0, react_1.useCallback)(async (id, updates) => {
        try {
            const response = await fetch(`${apiBasePath}/annotations`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const deleteAnnotation = (0, react_1.useCallback)(async (id) => {
        try {
    ...
    const createComment = (0, react_1.useCallback)(async (annotationId, text, parentId, imageUrl) => {
        try {
    ...
    const deleteComment = (0, react_1.useCallback)(async (id, annotationId) => {
        try {
